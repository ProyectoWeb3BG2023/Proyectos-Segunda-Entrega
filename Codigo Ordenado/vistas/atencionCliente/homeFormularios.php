...
$f = $_SESSION["suma"];
 ...
 foreach( $result as $datitos ) {
    ...
    $arreglo = ["$ic", "$prn", "$sn", "$pa","$sa","$d", "$td","$m","$c","$p","$e"];

    $_SESSION["datosCliente$f"] = $arreglo;
    var_dump($_SESSION["datosCliente$f"]);
    $f++;
    $_SESSION["suma"]++;
 }
...
<body>
    ...
    <section class="contenedorFormularios">
        ...
        // foreach( $containerArrays as $row ){
            ...
            $a = 1;
            $_SESSION["suma"] = $a;
        for( $a ; $a <= $cantidadClientes; $a++, $_SESSION["suma"]++ ) {
            ...
            if( $i == 1 ) {
                ...
                //  echo '    <h3>' . $_SESSION['tipoDocumento'] . '</h3>';
                   echo '    <h3>' . $_SESSION["datosCliente$a"][0] . '</h3>';
                 if ( $td == 'RUT' ){
